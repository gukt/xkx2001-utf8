...
class TestGiveSuccess:
    def test_transfers_item_from_player_to_npc_container(self, tmp_path: Path) -> None:
        world, player_id, npc = _load(tmp_path)
        yard = world.require_component(player_id, Position).room
        assert execute_line(world, player_id, "get 石头")
        assert _item_in(world, player_id, "石头") is not None
        assert _item_in(world, yard, "石头") is None
...
class TestGiveFailures:
    ...
    def test_npc_without_container_cannot_receive(self, tmp_path: Path) -> None:
    ...
    def test_no_drop_item_is_rejected(self, tmp_path: Path) -> None:
        doc = {
            "rooms": {
                "yard": {
                    "name": "院子",
