...
``SceneLoadError`` 定义在本模块：``CapabilitySpec.from_yaml`` 解析失败需要它，
但 scene_loader 要消费注册表，为避免 scene_loader <-> capabilities 循环依赖，
错误类型放在本模块；scene_loader 重新导出以保持 ``from mud_engine.scene_loader
