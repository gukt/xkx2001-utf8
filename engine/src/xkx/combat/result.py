...
# M3-1 练功（ADR-0032 决策 2）：dazuo/tuna busy condition handler 产出的资源变更
KIND_NEILI = "neili"  # neili 变化（dazuo 增长，clamp max_neili*2）
KIND_RESPIRATE = "respirate"  # jingli 增长（tuna，clamp max_jingli*2，区别于 KIND_JINGLI）
KIND_MAX_NEILI = "max_neili"  # max_neili += amount（amount=0 瓶颈只重置）+ neili 重置为 max_neili
KIND_MAX_JINGLI = "max_jingli"  # max_jingli += amount + jingli 重置（tuna 瓶颈）
KIND_EFF_JINGLI = "eff_jingli"  # eff_jingli += amount（tuna 瓶颈增长）
