...
YAML 解析失败抛 ``openmud.errors.SceneLoadError``（叶子错误模块，避免本模块
...
from openmud.semantic_color import validate_markup
...
def _parse_detail_aliases(
    raw_aliases: object, *, key: str, label: str, scene_path: Path
) -> tuple[str, ...]:
    if raw_aliases is None:
        return ()
    if not isinstance(raw_aliases, list):
        raise SceneLoadError(
            f"场景文件 {scene_path} 的{label}的 details[{key!r}].aliases 应是列表，"
            f"实际是 {type(raw_aliases).__name__}"
        )
    aliases: list[str] = []
    for i, item in enumerate(raw_aliases):
        if not isinstance(item, str) or not item:
            raise SceneLoadError(
                f"场景文件 {scene_path} 的{label}的 details[{key!r}].aliases[{i}] "
                f"应是非空字符串，实际是 {item!r}"
            )
        aliases.append(item)
...
def _parse_room_details(
    ...
    """``details``：K2 ``{id: {text, aliases?}}``；旧 ``{键: 纯字符串}`` 自动转换。"""
    ...
    for key, value in raw.items():
        ...
        loc = f"场景文件 {scene_path} 的{label}.details[{key!r}]"
        if isinstance(value, str):
            # 旧写法：键→纯字符串 → DetailEntry(text=值, aliases=(键,))
            validate_markup(value, location=loc)
            entries[key] = DetailEntry(text=value, aliases=(key,))
            continue
        if isinstance(value, Mapping):
            if not _DETAIL_KEY_RE.fullmatch(key):
                raise SceneLoadError(
                    f"{loc} 的 K2 主键应是无空格英文 id（如 shi_shi），实际是 {key!r}"
                )
            text = value.get("text")
            if not isinstance(text, str) or not text:
                raise SceneLoadError(
                    f"{loc}.text 应是非空字符串，实际是 {text!r}"
                )
            validate_markup(text, location=f"{loc}.text")
            aliases = _parse_detail_aliases(
                value.get("aliases"), key=key, label=label, scene_path=scene_path
            )
            unknown = set(value) - {"text", "aliases"}
            if unknown:
                raise SceneLoadError(
                    f"{loc} 含未知字段 {sorted(unknown)}"
                )
            entries[key] = DetailEntry(text=text, aliases=aliases)
            continue
        raise SceneLoadError(
            f"{loc} 应是字符串或 {{text, aliases?}} 映射，实际是 {type(value).__name__}"
        )
    if not entries:
        return None
...
def _ser_room_details(c: RoomDetails) -> dict:
    return {
        "entries": {
            key: {"text": entry.text, "aliases": list(entry.aliases)}
            for key, entry in c.entries.items()
        }
    }


def _des_detail_entry(raw: object) -> DetailEntry:
    if isinstance(raw, str):
        return DetailEntry(text=raw, aliases=())
    if isinstance(raw, Mapping):
        text = str(raw.get("text", ""))
        aliases_raw = raw.get("aliases", ())
        if isinstance(aliases_raw, str):
            aliases: tuple[str, ...] = (aliases_raw,)
        else:
            aliases = tuple(str(a) for a in list(aliases_raw or ()))
        return DetailEntry(text=text, aliases=aliases)
    return DetailEntry(text=str(raw), aliases=())


def _des_room_details(d: dict) -> RoomDetails:
    raw = d.get("entries", d)
    return RoomDetails(
        entries={str(k): _des_detail_entry(v) for k, v in dict(raw).items()}
    )
...
def _allowed_local_nature_phases(scene_path: Path) -> frozenset[str]:
    ...
    from openmud.nature import DEFAULT_PHASES
    from openmud.scene_loader import read_nature_config
