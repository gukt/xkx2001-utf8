...
from openmud.death_flow import UNCONSCIOUS_BLOCKED_VERBS
from openmud.events import Deny, run_vetoable
from openmud.intent import Intent
from openmud.library import (
    continue_more,
    find_book,
    format_toc,
    set_reading,
...
from openmud.messaging import publish_channel, room_say
from openmud.npc_query import is_askable_npc
from openmud.quest import accept_quest, try_complete_quest_on_give
from openmud.room_details import resolve_detail
from openmud.transfer import item_weight, transfer
...
# 转移域符号请从 ``openmud.transfer`` 导入，不再经本模块空壳转发。
...
def _cmd_look(world: World, player_id: EntityId, intent: Intent) -> list[str]:
    ...
    if description.outdoors:
        from openmud.nature import outdoor_desc_for_room
    ...
    from openmud.ferry import ferry_status_line
...
def _cmd_learn(world: World, player_id: EntityId, intent: Intent) -> list[str]:
    ...
    from openmud.ai import condition_from_data
    from openmud.conditions import evaluate
...
def _learn_deny_reason(condition: object, ctx) -> str:
    from openmud.conditions import And, Equals, Gte, Or, Predicate
...
def _cmd_attack(world: World, player_id: EntityId, intent: Intent) -> list[str]:
    ...
    from openmud.combat_system import try_engage
...
def _cmd_flee(world: World, player_id: EntityId, intent: Intent) -> list[str]:
    ...
    """
    from openmud.combat_system import attach_combat_system, clear_engagement, resolve_one_strike
...
def _invoke_room_hook_action(
    ...
    """
    from openmud.room_hooks import RoomHookContext, get_room_hook
...
class _JoinContext:
    ...
    def __init__(self, world: World, player_id: EntityId) -> None:
        from openmud.nature import nature_snapshot_for_room
...
def _join_deny_reason(condition: object, ctx: _JoinContext) -> str:
    ...
    from openmud.conditions import Equals, Predicate
...
def _exit_label(world: World, room: EntityId, direction: str) -> str:
    ...
    """
    from openmud.directions import exit_display_label
