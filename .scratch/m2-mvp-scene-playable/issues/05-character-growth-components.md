...
- [ ] `Vitals`/`BaseAttributes`/`SkillLevels`/`SkillProgress` 组件形状落地，作为三个独立 dataclass（不合并）。
- [ ] 三者的存档序列化/反序列化走 01 号票的注册表 codec（`to_dict`/`from_dict`），运行时可变（气血会变化）进存档。
- [ ] YAML 加载：NPC/玩家段可声明 `vitals:`（气血/内力/精力起始值+上限）、`attributes:`（四维属性）、`skills:`（初始已学技能+等级/经验，供门派 NPC/野外怪物预置战斗力）；缺省时给出合理默认值（不强制每个实体都声明）。
- [ ] `status` 命令：展示玩家当前 `Vitals`（三种资源 当前值/上限）+ `BaseAttributes`（四维，中文展示）；玩家实体若缺这些组件给出明确提示而非崩溃（防御性——理论上玩家实体在 M2 场景加载时应始终带这些组件，但测试要覆盖缺失路径）。
- [ ] `skills` 命令：展示 `SkillLevels.levels` 里每项技能的等级/经验；未学会任何技能时给出"你还没有学会任何技能"一类提示。
- [ ] 命令层测试（`execute_line`/`commands.execute` seam）：给定预置 `Vitals`/`BaseAttributes`/`SkillLevels` 的测试玩家，断言 `status`/`skills` 输出内容与状态查询一致。
- [ ] 存档往返测试：`Vitals`（气血变化后）/`SkillLevels`（学会技能后）save→restore 后数值一致。
- [ ] 现有测试全绿不回归。
