...
- **块 G（NPC 战斗行为 + Spawn 修复）**：`Behaviors` 新增 `aggro` kind（复用块 A 的交战建立逻辑，不新起一套）+ 把低频 Spawn/Reset 扫描从"聚合存活实例"改为"模板注册表驱动"，修复模板全灭后扫描失效的坑。
