...
> **引擎现状对照**：当前 Python 引擎仅有同房间 `room_say`（`engine/src/mud_engine/messaging.py`）；频道 / 登录按 [ADR-0008](../../docs/adr/0008-single-player-channel-login-out-of-stop-scope.md) 不在 M3 停机门闩内。
...
引擎对照：`mud_engine.messaging.room_say` ≈ LPC `say` 侧；**尚无** `channel:*` 对等物。
...
| 引擎房间消息 | `engine/src/mud_engine/messaging.py` |
