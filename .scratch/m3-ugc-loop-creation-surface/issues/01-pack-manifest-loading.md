...
- [x] `PackManifestError` 新增于 `openmud/errors.py`（与 `SceneLoadError` 平级，`errors.py` 模块 docstring 顺带更新一句提到两个错误类型都在这里），并加入该模块的 `__all__`。
...
- [x] `openmud.pack` 模块 docstring 说明本模块与 `scene_loader` 的关系（manifest 是包身份，`scene_loader` 是包内容，两者是独立的校验阶段——对齐 spec A1 决策的理由），供后续 02 号票的读者一眼看懂分工。
