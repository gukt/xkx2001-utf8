...
- [ ] `skills.py` 的 `SkillBehavior` Protocol：`hit_by(self, ctx: CombatContext) -> None` 改为 `hit_by(self, ctx: CombatContext) -> str | None`；`post_action(self, ctx: CombatContext) -> None` 改为 `post_action(self, ctx: CombatContext) -> str | None`；返回字符串即为本次调用要追加的播报片段，`None` 表示无追加。
...
- [ ] `hit_ob` 签名不变（已经能返回 `int | str | None`）。
- [ ] `skills.py` 里现有的示范钩子 `DemoPoisonStrikeBehavior.hit_by`（当前调用 `append_round_fragment("毒素渗入伤口！")`）改为 `return "毒素渗入伤口！"`。
- [ ] `test_skill_behavior_hooks.py` 里对 `hit_by`/`post_action` 断言方式同步更新——从"检查全局列表内容"改为"检查 `resolve_attack` 返回的 `CombatRoundResult.fragments` 是否包含预期片段"。
- [ ] 新增一条纯函数直测：连续两次独立调用 `resolve_attack`（给定同一份输入）互不污染，结果一致——专门断言这个改动恢复的纯函数属性。
- [ ] `just test` 全绿，尤其 `test_combat.py`、`test_skill_behavior_hooks.py`、`test_combat_engagement.py`。
