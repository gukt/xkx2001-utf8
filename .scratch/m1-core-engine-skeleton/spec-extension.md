...
- **块 C（物品系统）**：能力维度组件化（`Stackable`/`Valuable`/`Equippable` 占位/`Consumable` 占位，按需挂载）+ 转移统一原语 `transfer` + reject 校验钩子 + 堆叠合并/拆分 + 标志位 `no_get`/`no_drop`（后者支持字符串自定义提示）+ 容器物品（put/take from 容器）+ look 物品增强 + 重量与容量上限最小版。
...
4. 作为引擎开发者，我想物品路径预留 `on_get` / `on_drop` 事件点（可否决），空挂调用，以便"任务物品不能丢""诅咒物品拿不起"这类规则有触发点。
...
25. 作为终端玩家，我想某些物品拿不起来（`no_get`，如固定家具）或丢不掉（`no_drop`，且 `no_drop` 支持自定义提示文案，如"这是任务物品，不能丢弃"），以便任务物品与固定场景物件有合理行为。
...
- 物品路径 `on_get` / `on_drop`（均可否决）挂在 `_cmd_take` / `_cmd_drop` 前后。
...
- `TransferResult` 携带成功/失败 + 失败原因（`no_get` / `no_drop` / `over_capacity` / `over_weight` 等），由调用方翻译成玩家提示。
- reject 校验钩子（对应 LPC `reject` 机制）挂 `on_get` / `on_drop` 事件点，可否决转移。
...
**标志位 `no_get` / `no_drop`（C4）**

- 物品可挂标志位组件或字段：`no_get`（拿不起，如固定家具）、`no_drop`（丢不掉，且 `no_drop` 支持字符串自定义提示，如"这是任务物品，不能丢弃"）。quest 物品刚需。
...
- **覆盖点**：
  ...
  - 块 C：能力组件按需挂载；转移原语 take/drop/put/give 走同一路径且 reject 正确；堆叠合并/拆分；no_get/no_drop（含自定义提示）；容器 put/take from；look 物品 long+内容+数值；超重/超容量拒绝。
