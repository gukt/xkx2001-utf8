...
- `ask <npc> about <topic>`：解析层 NPC 候选用同房间 `Position` 实体（排除玩家），topic 是自由 token。
- NPC 响应支持 inquiry 映射（topic -> 字符串响应）+ 钩子（`DialogueHandler.on_topic`，M1 钩子可选，默认走声明式映射）。对应 LPC `ask`（LPC 无 talk 命令）。
