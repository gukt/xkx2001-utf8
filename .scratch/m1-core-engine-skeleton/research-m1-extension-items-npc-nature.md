...
**M1 必做**：
- **A1. 事件点/钩子环绕层（空挂）**：`on_tick` 订阅分发（`TickLoop.advance` 加 `dict[subscriber, handler]` 遍历，最核心）、`on_command_before`/`on_command_after`（`execute` 外环绕）、`on_before_enter_room`/`on_enter_room`/`on_leave_room`（`_cmd_go` 前后）、`on_take`/`on_drop`、`on_door_state_change`。空挂成本极低，不挂则 M2 改引擎接口。
...
- **C4. 标志位 `no_take`/`no_drop`**（no_drop 支持字符串自定义提示，quest 物品刚需）。
