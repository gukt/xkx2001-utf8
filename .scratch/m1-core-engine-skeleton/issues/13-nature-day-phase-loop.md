...
- [ ] 存在 `NatureState`（或等价 world 级状态），持有当前相位与推进进度，不进存档
- [ ] day_phase 配置数据驱动（YAML 或等价），含 phase 名 / length / time_msg / desc_msg
- [ ] 挂 `on_tick` 按可配比例推进相位（默认 60:1）
- [ ] 重启/构建时按可注入时钟对齐当前相位（测试不依赖墙钟）
- [ ] 默认四相 dawn/day/dusk/night 可用；题材包可换序列
- [ ] 调 `tick_loop.advance()` 快进到目标相位，断言相位切换
- [ ] 现有测试全绿（不回归）
