...
| **物品 / NPC 槽位补刷** | **已支持**：房间 `objects`（模板键 → 数量）为放置权威；模板 `respawn` 控制销毁后是否补刷；登记实例仍在（背包 / 别房也算）则占名额。见 [ADR-0010](adr/0010-room-centric-objects-placement.md) 与 [pre-m4-channels-spawn-quest](../.scratch/pre-m4-channels-spawn-quest/)。**多套 objects 抽一套（C11）已支持**：槽位 `{ random_of: [...], count? }`，补刷期重新抽签（非出口加载期 `random_of`）。 | 勿再写 `placed_in` / `in_room`；门钥匙等唯一引用物品不得 `objects` 合计 `>1` 或 `respawn: true`；勿把出口 `random_of` 当成放置随机。 |
