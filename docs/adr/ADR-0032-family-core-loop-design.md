...
**attempt_apprentice 钩子**（NpcDef 扩展）：声明式入门条件（技能等级/属性/任务标记/combat_exp），引擎求值（复用层1 谓词 ADR-0016）。条件数据是内容生产（YAML 声明），求值引擎是新能力。
...
4. **独立 LLM 选型**：Claude API + Langfuse（开放问题 1 已裁决）vs 其他？**倾向**：Claude API（03 §六技术选型）+ Langfuse 追踪。
