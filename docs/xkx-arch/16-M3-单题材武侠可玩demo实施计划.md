...
- **需 ADR**：ADR-0032 内容审核 pipeline MVP
...
| ADR-0032 | 内容审核 pipeline MVP | 03 §八 | M3-3 |
| ADR-0033 | 版权清洗策略（71 文件） | 03 §八 | M3-4 |
| ADR-0034 | 全仿真确定性决策点评估 | 05 §五 dissent 1 | M3-5 |

> ADR-0031（M3-2）是 Wave 1 前置，必须先写。ADR-0032~0034 可随对应 Wave 启动时写。
...
- [ ] ADR-0031 CPK 格式固化（Wave 1 前置，待写）
- [ ] 用户评审本计划文档 + 确认 M3 范围与开放问题

### 开放问题（需用户决策）

1. **M2 独立 LLM 验证状态**：阶段 -1 copilot 验证有范式污染偏差，M2 独立 LLM + Langfuse 是否已做？M3 内容生产用 Agent+DSL 为主还是人工转译为主？
   - 建议：若 M2 未完成，M3 Wave 2 先用独立 LLM 生产 1 门派内容 + Langfuse 追踪修订量（兼顾 M2 验证与 M3 内容生产）。
2. **门派选择**：M3 选哪个门派做完整内容？
   - 建议：优先扩展现有微场景（xueshan_micro 6 文件最大），版权清洗示范选 1 个金庸衍生门派。需盘点门派版权归属。
3. **可玩 demo 交付形态**：CLI REPL 够不够，还是要 WS 客户端（HTML/JS）？
   - 建议：CLI REPL 够内部验证；WS 客户端后置外部玩家测试（触发 PG 迁移）。
4. **版权清洗范围**：M3 做处理方案 + 1-2 示范，还是全部改完？
   - 建议：方案 + 1-2 示范，全量后置（03 §十硬约束 7 是"对外发布前必须处理"，M3 是内部 demo 不强制全量）。
5. **CPK provenance**：M3 固化还是后移门 3？
   - 建议：后移门 3（03 §四明确"首次对外发布前强制回填"，M3 开发期简单版本号）。
