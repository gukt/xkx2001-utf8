...
| ADR-0036 | 内容生产 LLM 选型（火山方舟 + deepseek-v4-flash，Langfuse 后置） | 03 §六 / ADR-0032 决策 6 | M3-1 子任务 4 |
...
1. **M2 独立 LLM 验证状态**：阶段 -1 copilot 验证有范式污染偏差（Agent = 本 session LLM，24.5% 修订量），M2 独立 LLM + Langfuse 真验证此前未完成。
   ...
   - **实施期变更**（2026-07-13，[ADR-0036](../adr/ADR-0036-content-llm-volcano-ark-langfuse-postpone.md)）：LLM 改火山方舟 Endpoint + deepseek-v4-flash（非 Claude API）；Langfuse 后置，本轮用 measure_revision 本地度量 semantic_ratio 产出首个 kill criteria 5 数据点。本轮子集先行验证管线，完整内容下一 session 扩展。
