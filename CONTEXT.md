...
**藏书**:
题材包内可检索、选定并付费阅读的书档内容；玩家主路径含书架 TOC、书名缩写解析、选书、按章付费阅读与分页展示。官方验收以扬州藏书阁为准。本波另含同房拦截练功（`practice`），以及房间旗标在藏书阁上的声明与生效（见「房间旗标」）。
...
**剧情门**:
相对标准门锁的声明式扩展：可耗钥解锁、未开时无该向可走出口、可声明「某 NPC 在场则挡某向」。本波验收以扬州翰林后院（或同构房）三件套为准；不另开任意脚本随时改出口的通用 API。
_Avoid_: 与标准门平行的第二套门系统, 本波把通用 add_exit/remove_exit 脚本面写进创作者契约作必达
...
**Pre-M4 引擎房间保真**:
M3 停机加固**整体完成之后**、开启 M4 **之前**插入的独立 effort。硬门闩三项：房间风景（`details`）、语义色 markup、完整藏书。本波必做但非硬门闩：日间店铺、剧情门（翰林三件套）。落地时同步回写创作者契约 v0 与 `--validate`/`--strict`（只做加法）。验收扩展官方扬州 MVP 场景（藏书阁、打铁铺日间店、翰林三件套房、带语义色/`details` 的户外等），不新建橱窗包。**后置**：液体灌装/饮用、防拐带 `valid_leave`。**放置模型不在本 effort**：已由兄弟批 + ADR-0010 收口。底稿见 `.scratch/pre-m4-engine-room-fidelity/`。
_Avoid_: 并入 M3 停机加固 S0/B3, 当成 post-MVP backlog, 与 GAP 台账（文档）混为一谈, 未 grill 直接实现, 重开 `placed_in`/`in_room` vs `objects`, 把仍可裁剪项或「本波必做非门闩」项升格为硬门闩, 把藏书误当成仅 `details` 文案, 缺藏书仍宣称 effort 关闭, 本波实装液体子系统, 新字段先透传不进契约, 另建房间保真橱窗包作唯一验收
