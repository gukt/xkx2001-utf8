...
**Channel**:
跨房间、按订阅投递的命名广播管道；与同房间发言（`room_say`）并列，不是其子类。预置两条：`chat`（玩家命令 `chat <text>`）与 `system`（仅运行时/API 可写）；创建 `PlayerSession` 时默认订阅两者；本波无 `tune`。`system` 不可由玩家命令写入。
_Avoid_: 把房间 say 叫频道, 把系统公告挤进玩家闲聊, rumor/门派/巫师频道表进核心, 未知命令 fallthrough 命中频道 ID, 假多人 seam 等同登录/联网

**房间 objects 放置**:
房间声明「模板键 → 数量」的物体清单；物品与 NPC 共用登记槽位补刷心智。实例仍存在（背包或其他房间也算）则占名额，仅销毁后才可按模板 `respawn` 补齐。权威写法见 ADR-0010。
_Avoid_: 以 `placed_in`/`in_room` 为权威放置字段, 把「离开地面」当成产生补刷缺口, 在房间保真 effort 重开放置模型

**Quest（声明式旗标任务）**:
题材包 YAML `quests.<id>` 声明的可接取任务：接取走 `quest accept <id>`（可要求同房 NPC）；完成靠对目标 NPC `give <物品> to <人物>` 交物与/或旗标；奖励本波至少给钱；到房只作前置。`ask` 不承担接取副作用。官方示例一条：`escort_delivery`。
...
**Pre-M4 引擎房间保真**:
M3 停机加固**整体完成之后**、开启 M4 **之前**插入的独立 effort：对照 LPC 房间写法补引擎能力（房间风景/`item_desc`、语义色 markup、`day_shop`、剧情门、液体灌装等；书院读书是否入波待 grill）。**放置模型不在本 effort**：已由兄弟批 + ADR-0010 收口。底稿见 `.scratch/pre-m4-engine-room-fidelity/`。
