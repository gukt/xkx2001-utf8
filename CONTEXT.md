...
**条件 DSL**:
`entry_guard` / `day_shop`（派生 `is_day`）/ `skills.*.learn_condition` / `npcs.*.behaviors[].when` 共用的结构化条件表达式（`predicate` / `field`+`value` / `gte` / `and`/`or`/`not`）。创作者可读规格见 `docs/condition-dsl.md`。不开放背包任意物、任务旗标等查询面；昼夜/雨读数可按房间 `local_nature` 贴纸合成（见「局部天气」），仍走既有谓词名。
_Avoid_: 在条件里发明 has_item/任务旗标/第二套局部天气谓词, 为单个玩法往通用 DSL 堆专用查询面（贵重物等走官方 hooks params）

**局部天气**:
房间可选静态贴纸 `LocalNature`（YAML `local_nature: {weather?: clear|rain, phase?: <相位表已有名>}`）。查询时合成户外 `look` 与条件 DSL `is_raining`/`is_night`/`is_day`/`phase`；已声明面用贴纸、未声明面回退 `World.nature` 单例（[ADR-0013](docs/adr/0013-local-nature-room-sticker.md)）。贴纸不随 tick/天气翻转演进；不增殖 `NatureState`；不做 region 树、天气→数值玩法。
_Avoid_: 每房一个可推进 NatureState, 邻房互见局部天气广播, 雨天减速等数值映射
