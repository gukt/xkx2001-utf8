...
**最后更新**：2026-07-23：Polishing Wave 4 落地（票 `06`）；fixed point `polishing-wave4-start`；code-review fix 已合。Next Up → Wave 5。**不自动开 M4**。

## 当前状态速览

- **阶段**：M0 完成；mvp-scope 10/10；**M1/M2/M3 里程碑可宣布完成**；**M3 停机加固整体完成**。**暂缓 M4**。Pre-M4 三批已关。**Polishing Wave 1–4 已落地**（`01`–`06` resolved）；其余 `07`–`13` 待做。
- **工作分支**：`feat/polishing`。
- **engine/**：测试绿（926）。
...
### 1. Polishing Wave 5（新 session）

**目标**：按 [.scratch/polishing/implement-plan.md](.scratch/polishing/implement-plan.md) Wave 5 提示词实现票 [`07`](.scratch/polishing/issues/07-condition-dsl-docs.md)（条件 DSL 文档）+ [`08`](.scratch/polishing/issues/08-liquid-eat-drink.md)（液体/eat/drink）。分支 `feat/polishing`；打 tag `polishing-wave5-start`。
...
- Polishing：[.scratch/polishing/](.scratch/polishing/)（Wave 1–4 已落地；工作分支 `feat/polishing`）。
