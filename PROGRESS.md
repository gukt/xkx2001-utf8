...
**最后更新**：2026-07-23：Polishing Wave 5 落地（票 `07`/`08`）；fixed point `polishing-wave5-start`；code-review fix 已合。Next Up → Wave 6。**不自动开 M4**。

## 当前状态速览

- **阶段**：M0 完成；mvp-scope 10/10；**M1/M2/M3 里程碑可宣布完成**；**M3 停机加固整体完成**。**暂缓 M4**。Pre-M4 三批已关。**Polishing Wave 1–5 已落地**（`01`–`08` resolved）；其余 `09`–`13` 待做。
- **工作分支**：`feat/polishing`。
- **engine/**：测试绿（935）。
...
### 1. Polishing Wave 6（新 session）

**目标**：按 [.scratch/polishing/implement-plan.md](.scratch/polishing/implement-plan.md) Wave 6 提示词实现票 [`09`](.scratch/polishing/issues/09-random-objects-pool.md)（随机 objects 表）+ [`10`](.scratch/polishing/issues/10-spawn-condition-hook-params.md)（刷怪条件扩展）。分支 `feat/polishing`；打 tag `polishing-wave6-start`。
...
- Polishing：[.scratch/polishing/](.scratch/polishing/)（Wave 1–5 已落地；工作分支 `feat/polishing`）。
