...
**最后更新**：2026-07-22：M3 停机加固 Wave 3 落地（票 `09`）；**Wave B3（`08`–`11`）全关**；加固整体（P0+B3）完成。673 绿。
...
1. **Pre-M4 频道/spawn/任务**（建议先）——新 session `/grill-with-docs`（输入 [.scratch/pre-m4-channels-spawn-quest/](.scratch/pre-m4-channels-spawn-quest/)），再 `/to-spec` → `/to-tickets` → `/implement`。切片见 [session-notes-2026-07-22.md](.scratch/pre-m4-channels-spawn-quest/session-notes-2026-07-22.md)。
...
3. 上述 Pre-M4 effort 关完（或 grill 明确缩 scope / 延期）后再决定是否开 M4——对照 [06](.scratch/mvp-scope/issues/06-scaling-commercialization-support-points.md) / [07](.scratch/mvp-scope/issues/07-governance-cost-tracking.md)。**不因 B3 做完自动滑入 M4。**
