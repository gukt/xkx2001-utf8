...
**最后更新**：2026-07-21：M2 Wave 0 `/code-review` fix 完成；下一环节 Wave 1。

## 当前状态速览

- **阶段**：M0 完成；mvp-scope 10/10；**M1 扩展验证矩阵已落地**；**M2 Wave 0 实现 + code-review fix 完成**，待 Wave 1。
- **工作分支**：`feat/m2-mvp-scene-playable`（Wave 0 fixed point tag：`m2-wave0-start`）。
- **engine/**：测试绿（431 passed）。`just verify-items` / `just verify-npc` / `just verify-nature`。
...
- [x] **M1 NPC 验证矩阵（D1–D5）**（2026-07-20）：[`just verify-npc`](justfile)。
...
1. **M2 Wave 1 `/implement`**：票 `05`–`09`（角色成长 / 死亡状态机 / 货币商店 / 门派框架 / 渡船）；开工前打 tag `m2-wave1-start`，见 [implement-plan.md](.scratch/m2-mvp-scene-playable/implement-plan.md)。
