...
**最后更新**：2026-07-19：M1 08 号票落地（命令生命周期钩子 before/after，[08](.scratch/m1-core-engine-skeleton/issues/08-command-lifecycle-hooks.md) resolved；`execute` 外包 `on_command_before`/`on_command_after`，`Allow`/`Deny`/`Replace` 三态 + 复用 07 的 `world.events`（给 `EventBus` 加 `handlers_for` 供调用方聚合）；212 测试全绿）。A 块 frontier 剩 [09](.scratch/m1-core-engine-skeleton/issues/09-domain-event-points.md)/[10](.scratch/m1-core-engine-skeleton/issues/10-condition-expression-evaluator.md)/[11](.scratch/m1-core-engine-skeleton/issues/11-yaml-unknown-section-passthrough.md)/[12](.scratch/m1-core-engine-skeleton/issues/12-component-field-tri-state.md)（09 复用同一事件总线地基）；mvp-scope 10/10 票全解决；M1 01~08 全 resolved。综合笔记 [.scratch/m1-core-engine-skeleton/research-m1-extension-items-npc-nature.md](.scratch/m1-core-engine-skeleton/research-m1-extension-items-npc-nature.md)。

## 当前状态速览

- **阶段**：M0 完成；M1 01~08 号票全 resolved（08 命令生命周期钩子 before/after）；M1 扩展 A 块 [07-12 六票](.scratch/m1-core-engine-skeleton/issues/) 中 07/08 已落地，frontier 剩 09/10/11/12；mvp-scope 10/10 票全解决（02 战斗/效果边界已拍板 [ADR-0004](docs/adr/0004-combat-effects-boundary-engine.md)）。下一步：A 块 frontier 09/10/11/12 `/implement`（09 复用同一 `world.events` 地基）；B/C/D 待 `/to-tickets`。
- **engine/ 现状**：`src/mud_engine/` 下 `world`/`components`/`commands`/`parsing`/`intent`/`matching`/`scenes`/`scene_loader`/`save`/`tick`/`events`/`cli`/`__main__`。场景数据 `engine/data/m1_default_scene.yaml`，存档 `engine/save/`（运行时产物，gitignore）。`python -m mud_engine` 跑通真终端闭环：移动/查看/拾取丢弃/容器/门与锁、方向与物品别名、静态 NPC 展示、周期+退出存档+崩溃恢复、on_tick 事件分发、命令 before/after 生命周期钩子（块 A 地基）。212 测试，`just gate` 全绿。
...
1. **M1 扩展 A 块 `/implement`（续）**：07/08 已落地（事件总线+on_tick / 命令 before/after 钩子）。A 块 frontier 剩 [09](.scratch/m1-core-engine-skeleton/issues/09-domain-event-points.md) 领域事件点（复用同一 `world.events` 地基）/ [10](.scratch/m1-core-engine-skeleton/issues/10-condition-expression-evaluator.md) 条件求值器 / [11](.scratch/m1-core-engine-skeleton/issues/11-yaml-unknown-section-passthrough.md) YAML 透传 / [12](.scratch/m1-core-engine-skeleton/issues/12-component-field-tri-state.md) 三态标注（均无前置可立即开始）。用 `/implement` 一次一个 ticket、清一次上下文。A 块完成后 `/to-tickets` 拆 B/C/D（B/C/D 相对独立可并行；A1 on_tick 给 B1 Nature 推进 + D1 NPC 行为，A2 条件求值器给 B/C/D 条件）。
