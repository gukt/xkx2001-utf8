...
**最后更新**：2026-07-21：M2 Wave 1 `/implement` 开工（tag `m2-wave1-start`）。

## 当前状态速览

- **阶段**：M0 完成；mvp-scope 10/10；**M1 扩展验证矩阵已落地**；**M2 Wave 0 完成**；**M2 Wave 1 进行中**（票 `05`–`09`）。
...
M2 Wave 1 `/implement`：票 `05`–`09`（角色成长 / 死亡状态机 / 货币商店 / 门派框架 / 渡船）；fixed point `m2-wave1-start`。
...
1. 完成本 Wave 1 实现 → `/code-review`（fixed point `m2-wave1-start`）→ fix → Wave 2。
