...
**最后更新**：2026-07-19（mvp-scope 02 号票拍板：战斗/效果边界归引擎，[ADR-0004](docs/adr/0004-combat-effects-boundary-engine.md)；mvp-scope 10/10 票全解决；M1 01~06 号票全 resolved，166 测试全绿）

## 当前状态速览

- **阶段**：M0 完成；M1 spec 已产出，`/to-tickets` 已拆出 6 张票（[.scratch/m1-core-engine-skeleton/issues/01~06](.scratch/m1-core-engine-skeleton/issues/)）；**01~06 号票全 resolved**（01 引擎骨架；02 解析执行解耦+别名；03 物品与容器；04 门与动态出口；05 心跳循环+存档崩溃恢复；06 YAML 场景 DSL）。mvp-scope 02 号票（战斗/效果边界）已拍板（[ADR-0004](docs/adr/0004-combat-effects-boundary-engine.md)），下一步：M2 `/to-spec`。
- **分支**：见当前 git 分支。
...
2. M1 里程碑收尾：01~06 号票全 resolved，准备 M2（一个 MVP 场景端到端可玩）`/to-spec`。
3. ~~mvp-scope 02 号票（战斗/效果边界）~~ 已拍板（[ADR-0004](docs/adr/0004-combat-effects-boundary-engine.md)，2026-07-19，用 `/design-an-interface`），M2 `/to-spec` 可直接推进。
