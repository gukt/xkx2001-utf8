...
**最后更新**：2026-07-22：Pre-M4 引擎房间保真 **spec 已发布**；下一步 `/to-tickets`（不自动开 M4）。

## 当前状态速览

- **阶段**：M0 完成；mvp-scope 10/10；**M1/M2/M3 里程碑可宣布完成**；**M3 停机加固整体完成**。**暂缓 M4**。Pre-M4：**① 频道/spawn/任务（已关闭）** → **② 引擎房间保真**（[spec.md](.scratch/pre-m4-engine-room-fidelity/spec.md) `ready-for-agent`；待 to-tickets）。
- **工作分支**：`feat/pre-m4-engine-room-fidelity`（自已合入 master 的频道/spawn/任务 tip 开出）。本地 `master` 含频道/spawn/任务 merge（是否 push `master` 另定）。
- **engine/**：测试绿（718）；契约 / GAP 见既有 docs；本批：[pre-m4-engine-room-fidelity/](.scratch/pre-m4-engine-room-fidelity/)。
...
### 1. Pre-M4 引擎房间保真：`/to-tickets` → implement

[spec.md](.scratch/pre-m4-engine-room-fidelity/spec.md)。下一步 `/to-tickets` → implement。
