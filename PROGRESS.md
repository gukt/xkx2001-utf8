...
1. **M1 扩展 A 块 `/implement`（续）**：07/08/09/10/11 已落地（事件总线+on_tick / 命令 before/after 钩子 / 领域事件点 / 条件求值器 / YAML 未识别段透传）。A 块 frontier 剩 [12](.scratch/m1-core-engine-skeleton/issues/12-component-field-tri-state.md) 三态标注（无前置可立即开始）。用 `/implement` 一次一个 ticket、清一次上下文。A 块完成后 `/to-tickets` 拆 B/C/D（B/C/D 相对独立可并行；A1 on_tick 给 B1 Nature 推进 + D1 NPC 行为，A2 条件求值器给 B/C/D 条件，A1 领域事件点给 B Nature 切换广播 / C 转移 reject 钩子 / D NPC 进房反应）。
