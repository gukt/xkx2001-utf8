...
**最后更新**：2026-07-22：Pre-M4 房间钩子 / 星宿机制 **`/to-tickets` 完成**（11 票，`01`–`11`，编号即依赖顺序）；不自动开 M4。下一步：`/implement` Wave 1（票 `01`）。

## 当前状态速览

- **阶段**：M0 完成；mvp-scope 10/10；**M1/M2/M3 里程碑可宣布完成**；**M3 停机加固整体完成**。**暂缓 M4**。Pre-M4：**① 频道/spawn/任务（已关闭）** → **② 引擎房间保真（已关闭）** → **③ 房间钩子 / 星宿机制**（`/to-tickets` 已完成，票 `01`–`11` 已发布；实现未开工）。
- **工作分支**：`feat/pre-m4-engine-room-fidelity`（已关，合并前仍可在此分支做 review fix）；下一 effort 建议新开 `feat/pre-m4-room-hooks-xingxiu`。
- **engine/**：测试绿（767）；下一步见 [pre-m4-room-hooks-xingxiu/implement-plan.md](.scratch/pre-m4-room-hooks-xingxiu/implement-plan.md)。
...
- Pre-M4 房间钩子 / 星宿机制 **Wave 1**：票 `01`（房间钩子协议 + 注册表 + 窄 `ctx` + 房间自由状态）。
...
### 1. Pre-M4 房间钩子 / 星宿机制：`/implement` Wave 1

按 [.scratch/pre-m4-room-hooks-xingxiu/implement-plan.md](.scratch/pre-m4-room-hooks-xingxiu/implement-plan.md) 开工，先做票 `01`（房间钩子协议 + 注册表 + 窄 `ctx` + 房间自由状态），全部后续机关票只阻塞于它。分支建议 `feat/pre-m4-room-hooks-xingxiu`。
...
- Pre-M4（加固后、M4 前）：[.scratch/pre-m4-channels-spawn-quest/](.scratch/pre-m4-channels-spawn-quest/)（**已关**）→ [.scratch/pre-m4-engine-room-fidelity/](.scratch/pre-m4-engine-room-fidelity/)（**已关**）→ [.scratch/pre-m4-room-hooks-xingxiu/](.scratch/pre-m4-room-hooks-xingxiu/)（票已发布 `01`–`11`；待 `/implement`）。
