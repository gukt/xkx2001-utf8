...
**最后更新**：2026-07-21：编辑器改判丢弃；创作者平台 + 留言板写入 post-MVP backlog（ADR-0006）。

## 当前状态速览

- **阶段**：M0 完成；mvp-scope 10/10；**M1/M2 完成**；**M3 前规格门禁已过**（[ADR-0005](docs/adr/0005-m3-ugc-loop-creation-surface.md)/[0006](docs/adr/0006-no-engine-editor-board-post-mvp-creator-platform.md)）；post-MVP 见 [.scratch/mvp-scope/post-mvp-backlog.md](.scratch/mvp-scope/post-mvp-backlog.md)。
- **工作分支**：`feat/m2-mvp-scene-playable`（Wave 5 fixed point tag：`m2-wave5-start`）。
- **engine/**：测试绿；`just verify-*` / `just verify-m2` 全绿。
...
1. M3 `/to-spec`：按 [03 Refinement](.scratch/mvp-scope/issues/03-ugc-dsl-design-inheritance.md) + [ADR-0005](docs/adr/0005-m3-ugc-loop-creation-surface.md) 拆「非官方内容包 → 加载 → 可玩」（引擎不做编辑器/留言板/Agent UI）。
