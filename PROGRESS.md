...
**最后更新**：2026-07-23：Polishing Wave 7 落地（票 `11`）；fixed point `polishing-wave7-start`；code-review fix 已合。Next Up → Wave 8。**不自动开 M4**。

## 当前状态速览

- **阶段**：M0 完成；mvp-scope 10/10；**M1/M2/M3 里程碑可宣布完成**；**M3 停机加固整体完成**。**暂缓 M4**。Pre-M4 三批已关。**Polishing Wave 1–7 已落地**（`01`–`11` resolved）；其余 `12`–`13` 待做。
- **工作分支**：`feat/polishing`。
- **engine/**：测试绿（961）。
...
### 1. Polishing Wave 8（新 session）

**目标**：按 [.scratch/polishing/implement-plan.md](.scratch/polishing/implement-plan.md) Wave 8 提示词实现票 [`12`](.scratch/polishing/issues/12-local-weather-adr.md)（局部天气继承 ADR，仅文档）。分支 `feat/polishing`；打 tag `polishing-wave8-start`。若 ADR 已提前写完可跳过。
...
- Polishing：[.scratch/polishing/](.scratch/polishing/)（Wave 1–7 已落地；工作分支 `feat/polishing`）。
