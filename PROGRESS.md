...
**最后更新**：2026-07-21：P1 排期拍板（W1/B3）；ADR-0009；hardening 同 spec 两 wave（P0→B3）。
...
- **engine/**：测试绿（649）；加固实现尚未开工（拍板只落文档）。
...
1. **M3 停机加固** `/to-spec` → `/to-tickets` → `/implement`（**同一 spec，两 wave**）：
   - **Wave P0（停机门闩 / S0）**
